...
public:
	...
	LevelEndLayout()
	{
		...
		int fbx = dialogX+tDialogBox->GetWidth()-(tFB->GetWidth()*2);
		cFB.constructRaw(fbx,dialogY,util.LEFT,util.TOP,"",tFB,tFB);

		
		//bg.setTexture("menu/th1.png");
		int midscreen = util.widthDisplay/2;
		int resetX = midscreen-(tReset->GetWidth()/2);
		int menuX = resetX-tLMenu->GetWidth();
		int next = resetX+tReset->GetWidth();
		cNext.constructRaw(next,dialogY+tDialogBox->GetHeight(),util.LEFT,util.TOP,"",tNext,tNext);
		cReset.constructRaw(resetX,dialogY+tDialogBox->GetHeight(),util.LEFT,util.TOP,"",tReset,tReset);
		cLMenu.constructRaw(menuX,dialogY+tDialogBox->GetHeight(),util.LEFT,util.TOP,"",tLMenu,tLMenu);
