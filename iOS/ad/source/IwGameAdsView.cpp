	...
	if (IW_GAME_ADS->isAdAvailable())
	{
		...
		if (IW_GAME_ADS->getError() <= MinError)
		{
			...
			if (data->Ad != NULL)
			{
				...
				{
					CIwList<CIwGameAdsViewAnimator*>::iterator it = data->Animators.begin();
					(*it)->setRestingPosition(IwGxGetScreenWidth()/2,data->AdSprite->getSrcHeight()/2);
				}
				data->Error = error;
				ResetAnims(data);
			}
...
void CIwGameAdsView::Draw()
{
	...
	for (int t = 0; t < NumAdsVisible; t++)
	{
		...
		if (data->Visible)
		{
			//Iw2DDrawImage(pFrame,CIwSVec2(data->Position.x-(pFrame->GetWidth()/2),data->Position.y-yFrame));//,CIwSVec2(data->AdSprite->getSrcWidth()+10,data->AdSprite->getSrcHeight()+10));
			
			//Iw2DDrawImage(pFrame,CIwSVec2(data->Position.x-(data->AdSprite->getSrcWidth()/2)-10,data->Position.y-yFrame),CIwSVec2(data->AdSprite->getSrcWidth()+20,data->AdSprite->getSrcHeight()+10));
			data->AdSprite->setPosition(data->Position.x, data->Position.y);
			data->AdSprite->setScale((iwfixed)(data->Scale * IW_GEOM_ONE));
			data->AdSprite->setAngle((iwangle)((data->Angle * IW_ANGLE_2PI) / 360.0f));
...
void CIwGameAdsView::Draw(CIw2DImage* pFrame,int yFrame)
{
	if (!Visible)
		return;
