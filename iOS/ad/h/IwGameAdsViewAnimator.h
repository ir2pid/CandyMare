...
public:
	...
	enum eIwGameAdsAnimPhase
	{
		...
		AnimPhaseDissapear,			// Animation done
		AnimPhaseDone,			// Animation done
	};

	// Properties
protected:
	CIwVec2						RestingPosition;	// resting position where ad should stop
	...
	int							DissapearDuration;	// Amount of time to ad shoud stay off screen
...
public:
	...
	void						setRestingPosition(int x)						{ RestingPosition.x = x; RestingPosition.y = 0; }
	...
	void						setDissapearDuration(int duration_ms)					{ DissapearDuration = duration_ms; }
...
protected:
	CIwVec2						TargetPosition;
	float						TargetAngle;
	float						TargetScale;
	CIwColour					TargetColour;
	CIwVec2						StartPosition;
	float						StartAngle;
	float						StartScale;
