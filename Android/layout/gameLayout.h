...
public:
	...
	GameLayout()
	{		
		util.flurryAnalytics.logLevelStart(storage.getLevel());
	...
	void calculateScore()
	{
		...
		util.flurryAnalytics.logLevelStop(level);
		...
		{
			util.flurryAnalytics.logLevelMove(level,stepNow);
		...
		if(storageStars<gObj.starAchived)
		{
			...
			if(storage.sStarDump[0] == 3)
				util.flurryAnalytics.logLevelStar(level,storage.sStarDump[0]);
		}
