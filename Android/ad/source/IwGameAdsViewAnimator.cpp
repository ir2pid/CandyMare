...
//	setRestingPosition((AdWidth * screen_scale) / 2, (AdHeight * screen_scale) / 2);
	...
	switch (AnimPhase)
	{
		...
		case AnimPhaseStay:
			State_PhaseStay();
			break;
		case AnimPhaseDissapear:
			State_PhaseDissapear();
			break;
		case AnimPhaseDone:
			break;
	}
...
void CIwGameAdsViewAnimator::Reset()
{
	...
	AnimTimer.setDuration(InDuration + OutDuration + StayDuration + DissapearDuration);
	AnimPhase = AnimPhaseIn;
}
...
void CIwGameAdsViewAnimator::State_PhaseStay()
{
	...
	{
		int wobble = (int)(sinf(((((elapsed - start_ms) * 720.0f) / 2000.0f) * PI) / 180.0f) * 25.0f);
...
void CIwGameAdsViewAnimator::State_PhaseOut()
{
	...
	elapsed = (int)AnimTimer.GetElapsedTime();
	if (elapsed >= InDuration + StayDuration +OutDuration)
		AnimPhase = AnimPhaseDissapear;

	if (AnimTimer.HasTimedOut())
		AnimTimer.Stop();

}
void CIwGameAdsViewAnimator::State_PhaseDissapear()
{
	if (AnimTimer.hasStopped())
		return;
